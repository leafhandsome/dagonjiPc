      ...
      <div class="index-getpwd dgj-container bgcfff clearfix">
        ...
        <div class="getpwd-content">
          ...
          <div v-if="active==1" class="ctx_two">
            <p v-if="!islogin && noLoginhasBind.bindPhoneStatus || islogin">
              <button @click="pwdResetType(1)" class="two_btn">通过手机短信验证码找回{{typepwd()}}密码</button>
            </p>
            <p v-if="!islogin && noLoginhasBind.setPwdSecurityStatus || islogin">
              <button @click="pwdResetType(2)" class="two_btn">密保信息找回{{typepwd()}}密码</button>
            </p>
            <p v-if="!islogin && noLoginhasBind.setAccountPwdStatus || islogin">
              <button @click="pwdResetType(3)" class="two_btn">{{type==0 ? '资金':'登录'}}密码找回{{typepwd()}}密码</button>
            </p>
            <p @click="goService">
          ...
          <div v-if="active==2" class="ctx_three">
           <div class="telcode" v-show="codetype==1">
             <form action="##">
               <p class="input">
                 ...
                 <input v-model="telobj.mobile" class="bz" placeholder="请输入已绑定手机号" type="text" v-if="!islogin && noLoginhasBind.bindPhoneStatus" disabled>
                 <input v-model="telobj.mobile" class="bz" placeholder="请输入已绑定手机号" type="text" v-if="islogin && hasBind.hasBindPhone" disabled>
               </p>
               <p class="input">
                 <span class="info">验证码：</span>
           ...
           <div v-show="codetype==2">
             <form action="##">
               ...
               <div class="ctx_three_btn">
                 <button @click="code_PwdQuestion" class="btn" type="button">提交</button>
           ...
           <div class="zijinpwd" v-show="codetype==3">
             <form action="##">
               <p class="input">
                 <span class="info">{{type==0 ? '资金':'登录'}}密码：</span>
                 <input v-model="accountPwd" class="bz" :placeholder="Pwdplaceholder[type]" type="password">
               </p>
               <div class="ctx_three_btn">
          ...
          <div v-if="active==3" class="ctx_four">
            <form action="##">
              <p class="input">
                ...
                <input class="bz" type="password" v-model="resetPwd.newPwd" :placeholder="placeholder[type][0]">
              </p>
              <p class="input">
                <span class="info">确认密码：</span>
                <input class="bz" type="password" v-model="resetPwd.confirmPwd" :placeholder="placeholder[type][1]">
              </p>
              <div class="ctx_three_btn">
                <button @click="submit_pwd" class="btn" type="button">提交</button>
                <!--<button class="btn reset" type="reset">重置</button>-->
              </div>
            </form>
          </div>
          <div v-if="active==4" class="ctx_five">
            ...
            <p v-if="type==0">修改成功，请点击<a href="javascript:;" @click="$router.push('/login')">登录</a></p>
            <p v-if="type==1">修改成功，请点击<a href="javascript:;" @click="$router.push('/user')">回到个人中心</a></p>
          </div>
        </div>
      </div>
      <!--加载动画-->
...
<script>
  ...
  export default {
    data () {
      return {
        msg: '忘记密码！',
        type:0,//0 找回登录登录  1 找回资金密码
        CodeObj:{},
        active:0, //步骤
        codetype:1,//步骤2的选择步骤 1短信找回 2密保找回 3资金/登录密码找回
        accountPwd:'',
        countnew:'',
        resetPwd:{newPwd:'',confirmPwd:''},
        ismsgcode_clk:true,
        user: {
          username: '',
          mobile:'',
        ...
        hasBind:'',
        Pwdplaceholder:["请输入您的资金密码","请输入您的登录密码"],
        placeholder:[['请输入新的登录密码','请再次输入新的登录密码'],['请输入新的资金密码','请再次输入新的资金密码']],
    ...
    mounted(){
      ...
      */
      ...
      if(this.type==1 && !this.islogin){
        this.$message.error('你没有登录，请登录后再找回资金密码！');
        this.logOut();
      }
    ...
    methods: {
      ...
      pwdResetType(num){
        switch(num){
          ...
          case 2:
            if(this.islogin){
              //已登录
              if(!this.hasBind.hasSetPwdQuestion){
                this.$message.warning('对不起，该账号没有设置过密保，请去个人中心设置！')
                return;
              }
            }
            this.getPasswordQuestion(num);
            break;
          case 3:
            if(this.islogin){
              //已登录
      ...
      getPasswordQuestion(num){
        this.loading.text='获取密保中...';
        this.loading.show=true;
        let username = '';
        if(!this.islogin && this.noLoginhasBind.setPwdSecurityStatus){
          username = this.noLoginhasBind.userName;
        }else if(this.islogin){
          username = this.user.username;
        }else if(!this.islogin && !this.noLoginhasBind.setPwdSecurityStatus){
          this.$message.warning('获取失败，请回到第一步验证用户是否存在！')
        }
        this.$api.getPasswordQuestion({userName:username}).then(res=>{
          if(res.result){
            let index = 0;
      ...
      code_PwdQuestion(){
        ...
        if(isnext){
          ...
          this.$api.validPasswordQuestion(params).then(res=>{
            if(res.result){
      ...
      code_accountPwd(){
        //登录/资金密码找回登录/资金密码
        if(this.accountPwd.length<=0){this.$message.error(this.Pwdplaceholder[this.type]);return;}
        if(!this.check_pwd(this.accountPwd)){this.$message.error('请输入6-20位的数字/字母的密码！');return;}
        ...
        if(this.type==0){
          ...
          this.$api.checkLoginPwd({password:this.accountPwd,userId:this.$store.getters.userId,token:this.$store.getters.token}).then(res=>{
            if(res.result){
      ...
      submit_pwd(){
        ...
        if(!this.check_pwd(this.resetPwd.newPwd)){this.$message.error('请输入6-20位的数字/字母的密码！');return;}
        if(!this.check_pwd(this.resetPwd.confirmPwd)){this.$message.error('请输入6-20位的数字/字母的密码！');return;}
        if(this.resetPwd.newPwd != this.resetPwd.confirmPwd){this.$message.error('请输入一致的密码！');return;}
        ...
        if(this.type==0){
          switch(this.codetype){
            case 1:
              this.$api.resetPwdByPhone({mobile:this.telobj.mobile,smsCode:this.telobj.code,newPwd:this.resetPwd.newPwd,confirmPwd:this.resetPwd.confirmPwd}).then(res=>{
                if(res.result){
                  this.$message.success('登录密码重置成功!请重新登录');
                  this.clearAllCookie();
            ...
            case 2:
              let params = {
                ...
                confirmPwd:this.resetPwd.confirmPwd
              }
              this.$api.resetPwdPasswordQuestion(params).then(res=>{
                if(res.result){
                  this.$message.success('登录密码重置成功!请重新登录');
                  this.clearAllCookie();
                  this.$store.commit('clearAll');
            ...
            case 3:
              ...
              this.$api.resetPwdByAccountPwd(options).then(res=>{
                if(res.result){
                  this.$message.success('登录密码重置成功!请重新登录');
                  this.clearAllCookie();
                  this.$store.commit('clearAll');
          ...
          if(!this.islogin){this.$message.error('你没有登录，请登录后再操作！');return;}
          this.$api.restAccountPwd({userId:this.$store.getters.userId,token:this.$store.getters.token,newPwd:this.resetPwd.newPwd,confirmPwd:this.resetPwd.confirmPwd}).then(res=>{
            if(res.result){
              this.$message.success('资金密码重置成功!');
              this.nextcheck();
            }else{
              this.remoteLogin(res);
            }
